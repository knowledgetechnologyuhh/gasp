...
class GradCAM:
    ...
    Generates saliency maps with respect to discrete emotion labels (the second last, fully-connected
    ...
    def grad_cam(self, x, list_y):
        ...
        for i, y in enumerate(list_y):
            ...
            # TODO (fabawi): seperate the backward and forward to maintain keep_graph=True when training just like
            ...
            for j, w in enumerate(grad_cam_weights):
                saliency_map += w * feature_maps[j, :, :]
...
'''
            ...
            if power == 0:
                if cumm_saliency_map is None:
                    cumm_saliency_map = torch.ones(feature_maps.size()[1:]).to(self._device)
                cumm_saliency_map = torch.mul(cumm_saliency_map, saliency_map)
            else:
                if cumm_saliency_map is None:
                    cumm_saliency_map = torch.zeros(feature_maps.size()[1:]).to(self._device)
                cumm_saliency_map = torch.stack((cumm_saliency_map, saliency_map.pow(power))).sum(dim=0)

            time.sleep(2)
            feature_maps = None
            output_activations = None
            one_hot = None
            grad_cam_weights = None
            saliency_map = None
            torch.cuda.empty_cache()
            print("here 3", i)

        if power == 0:
            cumm_saliency_map = torch.pow(cumm_saliency_map, 1 / len(list_y))
        else:
            cumm_saliency_map /= len(list_y)
            cumm_saliency_map = torch.pow(cumm_saliency_map, 1 / power)

        cumm_saliency_map /= torch.max(cumm_saliency_map)

        # Return the list of normalized saliency maps
        return cumm_saliency_map'''